...
const Buisness = () => {
  ...
  const [BuisnessPosts, setBuisnessPosts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const fetchBuisnessPosts = async () => {
      try {
        const postsCollection = collection(db, 'posts');
        const BuisnessPostsQuery = query(postsCollection, where('category', '==', 'Buisness'));

        const querySnapshot = await getDocs(BuisnessPostsQuery);
        const posts = querySnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data()
        }));

        setBuisnessPosts(posts);
        setLoading(false);
      } catch (err) {
        console.error('Error fetching Buisness posts:', err);
        setError('Error fetching Buisness posts');
        setLoading(false);
      }
    };
  ...
  return (
    <div className="Buisness-posts p-4 ">
      <Swiper
        slidesPerView={3}
        loop={true}
        autoplay={{ delay: 3000 }}
      ...
      >
        {BuisnessPosts.length > 0 ? (
          BuisnessPosts.map((post) => (
          ...
          <p>No Buisness posts available</p>
        )}
      </Swiper>
        ...
        <div  className={`ml-2 mt-20 w-auto h-auto space-x-16 ${theme === "dark" ? "text-gray-300 bg-gray-800" : "text-black bg-white"}`}>
          <h1 className="relative text-4xl font-myfont text-black text-center font-bold before:absolute before:top-1/2 before:left-10 before:w-[38%] before:h-[1px] before:bg-gray-600 before:content-[''] before:-translate-y-1/2 after:absolute after:top-1/2 after:right-10 after:w-[38%] after:h-[1px] after:bg-gray-600 after:content-[''] after:-translate-y-1/2">
            <span className={`${theme === "dark" ? "text-gray-300" : "text-black"}`}>Buisness Posts</span>
          </h1>
          {BuisnessPosts.length > 0 && (
         BuisnessPosts.map((post) => (
